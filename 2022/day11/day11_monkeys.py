...
class Monkey:
    def __init__(self, items, operation, divisible_test, true_monkey, false_monkey):
    ...
    def inspect_item(self, idx, worry):
        ...
        else: self.items[idx] = mpz(new_item) % mpz(9699690)
        # else: self.items[idx] = new_item # doesn't work with big numbers
...
def parse_data(filename):
    ...
    with open(filename) as f:
        ...
        for i in range(0, len(lines), 7):
            ...
            data.append(Monkey(items, operation, divisible_test, true_monkey, false_monkey))
